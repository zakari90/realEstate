...
export default function InvestmentMainTableComponent({
  investments,
}: {
  ...
  const totalPages =
    investments.length > 0 ? Math.ceil(investments.length / itemsPerPage) : 0;
    ...
    <div className="w-full overflow-auto" dir="rtl">
        ...
        <TableBody>
          {paginatedInvestments.map((investment, index) => (
            <TableRow key={index}>
              ...
              {visibleColumns.includes("numContributors") && (
                <TableCell>{investment.numContributors}</TableCell>
              )}
      ...
      <Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
        <DialogContent className="sm:max-w-[625px]" dir="rtl">
          <DialogHeader>
            <DialogTitle>
              {selectedInvestment?.title} - تفاعلات العملاء
            </DialogTitle>
          </DialogHeader>
...
function InvestmentDetails({ offer, index }: investmenOfferProps) {
  ...
  return (
    <>
      <TableCell>
        {offer.accepted === true ? (
          <PhoneCallLink phone={offer.clientPhone || ""} />
        ) : (
          <p>غير مقبول</p>
        )}
      </TableCell>
