...
import { getAgentPropertiesWithDetails, getAgentPropertyImages, getAgentPropertyOffers } from "@/app/_actions/actions";
...
export const propertySlice = createAppSlice({
  ...
  reducers: (create) => ({
    fetchProperties: create.asyncThunk(
      async () => {
        const response = await getAgentPropertiesWithDetails();
        return response;
      },
      {
        pending: (state) => {
